        ...
        stage('Sonar Code Analysis') {
            steps {
                withSonarQubeEnv('sonarqube') {
                    sh """${tool('sonar-scanner')} -X \
                        -Dsonar.projectKey=${env.BRANCH_NAME ?: 'master'} \
                        -Dsonar.projectName=${env.BRANCH_NAME ?: 'master'} \
        ...
        stage('Stash npm Dependencies') {
            steps {
                script {
                    ...
                    stash name: 'GitHubDir', includes: '/var/lib/jenkins/workspace/React_project/GitHub-/node_modules/**', allowEmpty: true
                }
            }
        }
